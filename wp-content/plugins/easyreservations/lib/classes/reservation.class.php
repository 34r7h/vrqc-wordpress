			...
			public function Calculate($history = false){
				...
				if($taxes && !empty($taxes)){
					...
					foreach($taxes as $tax){
						...
						else $tax_amount = $theprice / (1+$tax[1]/100);
					...
					if($mode < 3){
						...
						if($afterpersons){
							...
							if(RESERVATIONS_USE_TIME){
								...
								//TODO: fix
								$case = "Case When TIMESTAMPDIFF(SECOND,$arrival, $departure)>=$interval&&DATE($departure) = DATE('$startdate')$addend Then 0 Else 1 End";
