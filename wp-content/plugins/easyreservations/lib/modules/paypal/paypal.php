...
if(is_admin()){
	...
	function easyreservations_add_settings_tab(){ 
		if(isset($_GET['site']) && $_GET['site'] == "pay") $current = 'current'; else $current = '';
	...
	function easyreservations_pay_save_settings(){
		if(isset($_GET['site']) && $_GET['site'] == "pay" && isset($_POST['action'])){
			if($_POST['action'] == "reservation_pay_settings"){
				...
				if(isset($_POST['er_pay_ssl'])) $er_pay_ssl = 1;
				else $er_pay_ssl = 0;

				if(isset($_POST['deposit_activate'])) $deposit_activate = 1; else $deposit_activate = 0;
				if(isset($_POST['deposit_payperc'])) $deposit_payperc = 1; else $deposit_payperc = 0;
				if(isset($_POST['deposit_payfull'])) $deposit_payfull = 1; else $deposit_payfull = 0;
				if(isset($_POST['deposit_payown'])) $deposit_payown = 1; else $deposit_payown = 0;
				if(isset($_POST['deposit_percantage'])) $deposit_percantage = $_POST['deposit_percantage']; else $deposit_percantage = '';
				if(isset($_POST['discount_amount'])){
					$percamt_array = '';
					foreach($_POST['discount_amount'] as $key => $value){
						if(!empty($value)){
							if(isset($_POST['discount_type'][$key]) && $_POST['discount_type'][$key] == '%') $percamt_array[] = $value.'%';
							else $percamt_array[] = $value;
						}
					}
				}
				$deposit = array('on' => $deposit_activate, 'full' => $deposit_payfull, 'own' => $deposit_payown, 'perc' => $deposit_payperc, 'percamt' => $percamt_array);
		...
		if(isset($_GET['site']) && $_GET['site'] == "pay"){
			...
			if(!empty($options['deposit']['percamt'])){
				...
				foreach($array as $value){
					$deposit_options_value .= '<input type="text" name="discount_amount[]" value="'.str_replace('%', '', $value).'">';
					...
					$deposit_options_value .= '</select><br>';
				}
			}
			...
			if(!get_option('reservations_email_to_admin_paypal')) echo '<br><a href="admin.php?page=reservation-settings&site=email" style="color:#ff0000;font-weight:bold;text-decoration:underlined">'.__( 'Dont forget the new email types!' , 'easyReservations' ).'</a>'; ?>
			<script type="text/javascript">
				function erReturn(t,type){
					...
					if(select == 'useredit'){
						...
						var pagesel = ''; 
						if(type == 'dibs') pagesel = '<?php echo str_replace(array("'","\r","\n"), array('"', '', ''), wp_dropdown_pages(array('selected' => ((is_numeric($doptions['return_url'])) ? $doptions['return_url'] : 0), 'echo' => 0, 'name' => 'er_dibs_own_return_link' )));?>';
						else if(type == 'ogone') pagesel = '<?php echo str_replace(array("'","\r","\n"), array('"', '', ''), wp_dropdown_pages(array('selected' => ((is_numeric($ogoptions['return_url'])) ? $ogoptions['return_url'] : 0), 'echo' => 0, 'name' => 'er_ogone_own_return_link' )));?>';
						else if(type == 'wallet') pagesel = '<?php echo str_replace(array("'","\r","\n"), array('"', '', ''), wp_dropdown_pages(array('selected' => ((is_numeric($goptions['return_url'])) ? $goptions['return_url'] : 0), 'echo' => 0, 'name' => 'er_wallet_own_return_link' )));?>';
						else if(type == '2checkout') pagesel = '<?php echo str_replace(array("'","\r","\n"), array('"', '', ''), wp_dropdown_pages(array('selected' => ((is_numeric($coptions['return_url'])) ? $coptions['return_url'] : 0), 'echo' => 0, 'name' => 'er_2checkout_own_return_link' )));?>';
						else if(type == 'authorize') pagesel = '<?php echo str_replace(array("'","\r","\n"), array('"', '', ''), wp_dropdown_pages(array('selected' => ((is_numeric($aoptions['return_url'])) ? $aoptions['return_url'] : 0), 'echo' => 0, 'name' => 'er_authorize_own_return_link' )));?>';
						else if(type == 'pay') pagesel = '<?php echo str_replace(array("'","\r","\n"), array('"', '', ''), wp_dropdown_pages(array('selected' => ((is_numeric($options['er_pay_return'])) ? $options['er_pay_return'] : 0), 'echo' => 0, 'name' => 'er_pay_own_return_link' )));?>';
						document.getElementById('er_'+type+'_return_2nd').innerHTML = pagesel;
					} else if(select == 'own'){
						var pageown = '';
						if(type == 'dibs') pageown = '<?php echo (!is_numeric($doptions['return_url']) && $doptions['return_url'] != 'useredit') ? $doptions['return_url'] : 'http://'; ?>';
						else if(type == 'ogone') pageown = '<?php echo (!is_numeric($ogoptions['return_url']) && $ogoptions['return_url'] != 'useredit') ? $ogoptions['return_url'] : 'http://'; ?>';
						else if(type == 'wallet') pageown = '<?php echo (!is_numeric($goptions['return_url']) && $goptions['return_url'] != 'useredit') ? $goptions['return_url'] : 'http://'; ?>';
						else if(type == '2checkout') pageown = '<?php echo (!is_numeric($coptions['return_url']) && $coptions['return_url'] != 'useredit') ? $coptions['return_url'] : 'http://'; ?>';
						else if(type == 'authorize') pageown = '<?php echo (!is_numeric($aoptions['return_url']) && $aoptions['return_url'] != 'useredit') ? $aoptions['return_url'] : 'http://'; ?>';
						else if(type == 'pay') pageown = '<?php echo (!is_numeric($options['er_pay_return']) && $options['er_pay_return'] != 'useredit') ? $options['er_pay_return'] : 'http://'; ?>';
          ...
          return true;
				...
				function easy_add_deposit(){
					var newfield = '<input type="text" name="discount_amount[]" value="">';
					newfield += '<select name="discount_type[]"><option value="cur" selected="selected">&<?php echo RESERVATIONS_CURRENCY; ?>;</option><option value="%">%</option></select><br>';
	...
	function easyreservation_generate_payment_form($res, $price, $submit = false, $discount = false){
		...
		if(!empty($options)){
			if(count($options) > 1){
				...
				$script = <<<JAVASCRIPT
<script type="text/javascript">
	if(document.getElementById('depositbox')) document.getElementById('depositbox').style.display= 'none';
...
JAVASCRIPT;
	...
	function easyreservations_generate_paypal_button($res = false, $theprice = 0,  $link = false, $button = false, $discount = false, $type = 'paypal'){
		...
		if(is_array($res)){
			...
			$theid = substr($theid,0,-1);
			if(!isset($paypalOptions['multititle'])) $paypalOptions['multititle'] = "[nr] Reservations for [adults]+[childs] persons";
			$theTitle = str_replace(array('[nr]', '[persons]', '[adults]', '[childs]'), array($i,$persons,$adults,$childs), $paypalOptions['multititle']);
		} else {
			$res->Calculate();
		...
		if(!empty($price) && $price > 0){
			...
			} else $return  = apply_filters('reservations_generate_gateway_button', $res, $theid, $theTitle, $price, $nonce);
